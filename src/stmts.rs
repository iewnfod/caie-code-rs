use crate::{CpcResult, Expr, Flow, Interpreter, RuntimeValue, Span, Stmt};

impl Interpreter {
	pub fn execute(&mut self, stmt: Stmt) -> CpcResult<Flow> {
		let flow = match stmt {
			...
			Stmt::Return { value, .. } => {
				...
				match value {
					...
					None => Flow::Return(RuntimeValue::Null),
				}
			},
			Stmt::Break { .. } => {
	...
	pub fn if_stmt(&mut self, condition: Expr, true_body: Box<Stmt>, false_body: Option<Box<Stmt>>) -> CpcResult<Flow> {
	...
	pub fn for_stmt(&mut self, var_name: String, start: Expr, end: Expr, body: Box<Stmt>, span: Option<Span>) -> CpcResult<Flow> {
		...
		if let (RuntimeValue::Int(s), RuntimeValue::Int(e)) = (s, e) {
			...
			for i in s..=e {
				...
				match flow {
					...
					Ok(Flow::Return(val)) => return Ok(Flow::Return(val)),
					Err(e) => return Err(e),
					_ => (),
				}
			}
	...
	pub fn while_stmt(&mut self, condition: Expr, body: Box<Stmt>) -> CpcResult<Flow> {
		loop {
			let c = self.evaluate(condition.clone())?;
			let flag = self.runtime2bool(c);
			if flag {
	...
	pub fn repeat_stmt(&mut self, body: Box<Stmt>, condition: Expr) -> CpcResult<Flow> {
		loop {
			let flow = self.execute(*body.clone());
			match flow {
				Ok(Flow::Break) => return Ok(Flow::Normal),
